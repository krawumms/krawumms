...
$SCRIPTPATH/retry.sh docker pull krawumms/$DOCKER_IMAGE_NAME:$BUILD_TAG || echo "Could not pull docker build cache from TAG ${BUILD_TAG}!"
$SCRIPTPATH/retry.sh docker pull krawumms/$DOCKER_IMAGE_NAME:$SECONDARY_BUILD_TAG || echo "Could not pull docker build cache from TAG ${SECONDARY_BUILD_TAG}!"
